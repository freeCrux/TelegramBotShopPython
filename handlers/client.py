...
async def start_help(message: types.Message):
    sql_db.add_client(client_id=message.from_user.id)
...
async def show_available_products(message: types.Message):
    products: list = sql_db.get_available_product_list()
    if len(products) > 0:
...
async def buy_product(callback: types.CallbackQuery):
    product_id = int(callback.data.split(':')[1])
    product_price: int = sql_db.get_product_price(prod_id=product_id)
    balance: int = sql_db.get_client_balance(client_id=callback.message.chat.id)
    if sql_db.counter_deliveries_by_product(prod_id=product_id) < 1:
        await callback.message.answer("Увы весь товар закончился.")
        await callback.answer("Товар закончился")
    elif balance >= product_price:
        delivery_id = sql_db.get_id_available_deliver(prod_id=product_id)
        try:
            sql_db.change_client_balance(client_id=callback.message.chat.id, cash=(-1) * product_price)
            sql_db.register_new_sale(buyer_id=callback.message.chat.id, del_id=delivery_id, paid=product_price)
            sql_db.change_available_status_delivery(del_id=delivery_id)
            delivery_data: tuple = sql_db.get_delivers_from_product_id(prod_id=product_id)
...
async def show_client_info(message: types.Message):
    data: tuple = sql_db.get_herself_data_for_client(client_id=message.from_user.id)
...
async def show_sales(message: types.Message):
    sales: list = sql_db.get_sales_from_client_id(client_id=message.from_user.id)
    if len(sales) > 0:
...
async def show_sale_from_delivery_id(callback: types.CallbackQuery):
    ...
    delivery_data: tuple = sql_db.get_delivery_info_from_id(del_id=del_id)
    await show_delivery(client_id=callback.message.chat.id, delivery_data=delivery_data)
    await callback.answer()
